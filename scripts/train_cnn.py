"""Train a simple CNN model with LOSO evaluation."""
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Train CNN with LOSO protocol")
    ...
    dataset = GaitDataset(window_size=args.window_size, overlap=args.overlap, label_mode="sequence")
    ...
    for test_subject in subjects:
        ...
        model = SimpleCNN(in_channels=dataset.in_channels, num_classes=dataset.num_classes).to(device)
        ...
        best_loss = float("inf")
        epochs_no_improve = 0
        best_path = output_dir / f"best_cnn_subject_{test_subject}.pth"
